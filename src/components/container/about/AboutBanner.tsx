import React ,{useState,useEffect} from "react";

const AboutBanner = () => {
  const [aboutUsDatapinpint, setAboutUsDataPinPoint] = useState();
  const [aboutUsDatadescription, setAboutUsDatadescription] = useState();
  const [aboutUsDataheading, setAboutUsDataheading] = useState();
  const url = 'http://localhost:8000/aboutus';

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(url);
        if (response.ok) {
          const data = await response.json();
          setAboutUsDataPinPoint(data.pinpoint)
          setAboutUsDatadescription(data.description)
          setAboutUsDataheading(data.heading)
          console.log(data);
        } else {
          console.error('Error fetching data.');
        }
      } catch (error) {
        console.error('Error:', error);
      }
    };

    fetchData();
  }, []);
  return (
    <div className="banner-area pd-bottom-120 banner-small-inner bg-light bg-relative bg-cover">
      <div className="container">
        <h4>{aboutUsDatapinpint}</h4>
        <h3>
          {aboutUsDataheading}
        </h3>
        <div className="row">
          <div className="col-lg-6 mt-lg-4 pt-lg-2">
            <p>
              {aboutUsDatadescription} 
            </p>
          </div>
          <div className="col-lg-6 order-lg-first text-lg-center">
            <div className="scroll-down top_image_bounce">
              <a href="#about-area">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  xmlnsXlink="http://www.w3.org/1999/xlink"
                  width="40"
                  height="80"
                  viewBox="0 0 40 80"
                >
                  <image
                    id="scroll"
                    width="40"
                    height="80"
                    xlinkHref="data:img/png;base64,"
                  />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AboutBanner;
